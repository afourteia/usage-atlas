...
function heatmapMarkup(account, id) {
  ...
  const grid = `<div class="heatmap-grid" role="grid" aria-label="${escape(label)} token usage by day and hour, ${escape(timezone)}" aria-rowcount="${days + 1}" aria-colcount="25"><div class="heatmap-axis" role="row"><span role="columnheader" class="heatmap-date">Hour</span>${Array.from({ length: 24 }, (_, hour) => `<span role="columnheader" aria-label="${hour}:00 to ${hour + 1}:00" class="heatmap-hour ${hour % 3 === 0 ? "major" : ""}"><span class="heatmap-tick ${hour === 0 ? "first" : ""}" aria-hidden="true">${String(hour).padStart(2, "0")}</span>${hour === 23 ? '<span class="heatmap-end" aria-hidden="true">24</span>' : ""}</span>`).join("")}</div>${chart.rows
    .map(
      (row, index) =>
